...
import { PaginatedUsers, User, UserFilters, UserStats } from '../interfaces/user.interface';
...
export class UserService {
  ...
  loadUsers(): Observable<User[]> {
    ...
    return from(this.supabaseClient.from('users')
        .select(`
          id,
          email,
          full_name,
  ...
  getUserById(userId: string): Observable<User> {
    return from(this.supabaseClient.auth.admin.getUserById(userId)).pipe(
      map(response => {
        if (response.error) {
          throw new Error(response.error.message);
        }

        return fromSupabaseUser(response.data.user);
      }),
      catchError(error => {
  ...
  createUser(userData: {
    ...
    return from(this.supabaseClient.auth.admin.createUser({
      ...
      map(response => {
        ...
        const newUser = fromSupabaseUser(response.data.user);
  ...
  updateUser(userId: string, userData: {
    ...
    return from(this.supabaseClient.auth.admin.updateUserById(userId, updateData)).pipe(
      map(response => {
        if (response.error) {
          throw new Error(response.error.message);
        }

        const updatedUser = fromSupabaseUser(response.data.user);
  ...
  deleteUser(userId: string): Observable<boolean> {
    ...
    return from(this.supabaseClient.auth.admin.deleteUser(userId)).pipe(
      map(response => {
        if (response.error) {
          throw new Error(response.error.message);
        }
  ...
  toggleUserStatus(userId: string, isActive: boolean): Observable<User> {
    ...
    return from(this.supabaseClient.auth.admin.updateUserById(userId, {
      user_metadata: {
        is_active: isActive
      }
    })).pipe(
      ...
      finalize(() => this.loading.set(false)),
      catchError(error => {
        const errorMessage = error instanceof Error ? error.message : 'Error cambiando estado del usuario';
        this.error.set(errorMessage);
        return throwError(() => error);
      })
    );
